 ...
 */
MAX31865_RTD::MAX31865_RTD( SoftSPI &S, ptd_type type, uint8_t cs_pin, uint16_t rtd_rref ) : sSPI(S)
{
  /* Set the type of PTD. */
  this->type = type;
...
void MAX31865_RTD::reconfigure( bool full )
{
  ...
  if (full)
  {
    ...
    sSPI.transfer( ( threshold >> 8 ) & 0x00ff );
    sSPI.transfer(   threshold        & 0x00ff );
  ...
  digitalWrite( this->cs_pin, LOW );
  sSPI.transfer( 0x80 );
  sSPI.transfer( this->configuration_control_bits );
  digitalWrite( this->cs_pin, HIGH );
  */
...
uint8_t MAX31865_RTD::read_all()
{
  ...
  this->measured_configuration = sSPI.transfer( 0x00 );

  combined_bytes  = sSPI.transfer( 0x00 ) << 8;
  combined_bytes |= sSPI.transfer( 0x00 );
  ...
  this->measured_status = sSPI.transfer( 0x00 );
...
uint8_t MAX31865_RTD::fault_status()
{
  ...
  sSPI.transfer( MAX31865_FAULTSTAT_REG );
  fault_status = this->measured_status = sSPI.transfer( 0x00 );
  digitalWrite( this->cs_pin, HIGH );
  return( fault_status );
  */
}
...
uint8_t MAX31865_RTD::config_register()
{
  ...
  // Tell the MAX31865 that we want to read, starting at register 0. 
  sSPI.transfer( 0x00 );
  config_register = this->measured_status = sSPI.transfer( 0x00 );
  digitalWrite( this->cs_pin, HIGH );
