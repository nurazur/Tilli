...
#ifndef _SOFTMAX31865_H
#define _SOFTMAX31865_H
...
public:
  ...
  MAX31865_RTD( SoftSPI &, ptd_type type, uint8_t cs_pin, uint16_t rtd_rref =0 );
  ...
  double resistance( ) const  { return( (double)raw_resistance( ) * configuration_rtd_rref / (double)RTD_ADC_RESOLUTION ); }
  ptd_type cfg_type() const { return ( type ); }
  uint16_t cfg_rref() const { return ( configuration_rtd_rref ); }
  void enableBias(bool b);
  void set_cs_pin(uint8_t pin){this->cs_pin = pin;}
