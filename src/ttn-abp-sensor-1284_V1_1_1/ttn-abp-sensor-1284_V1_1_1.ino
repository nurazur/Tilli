...
// Version: 1.1.1
// Frame counter can be reset in Configuration tool.
// MAX31865 on SoftSPI
...
static FrameCounter FrmCnt((sizeof(NodeConfig)/64)*64 +64);
/*****************************************************************************/
...
Calibration CalMode(node, &Serial, FrmCnt, (uint8_t*) KEY);
/*****************************************************************************/
/***                            Sensor Drivers                             ***/
/*****************************************************************************/
...
#ifdef USE_MAX31865
#include <MAX31865_SoftSpi.h>
SoftSPI mySPI(sMOSI,sMISO,sSCK);
...
static void Start_MAX31865(void)
{
    ...
    rtd = new MAX31865_RTD( mySPI, MAX31865_RTD::RTD_PT1000, RTD_CS_PIN, 3900 );
    #endif

    #ifdef PT100
    // For PT 100  (Ref Ref on breakout board = 430 Ohms 0.1%)
    rtd = new MAX31865_RTD( mySPI, MAX31865_RTD::RTD_PT100, RTD_CS_PIN, 430 );
    //Serial.println("created new instance of Max31865 class.");
    #endif
}

static uint8_t Measure_MAX31865(Payload& msg);
static uint8_t Measure_MAX31865(Payload& msg)
{
    uint8_t status = 0xFF;
    if(rtd)
    {
        ...
        if (status==0 )
        {
            ...
            Serial.print(temperature, 2); Serial.println(F(" C" ));
        }
        digitalWrite(RTD_PWR_PIN, 0);
        pinMode(RTD_PWR_PIN,INPUT);
        //digitalWrite(RTD_CS_PIN, 0);
        pinMode(RTD_CS_PIN,INPUT);
    }
...
static uint8_t brightness_with_LDR(int8_t LDRPin)
{
    ...
    */
    ...
    if (enabled)
    {
        ...
        if ((state == 0 && trigger==FALLING) || (state==1 && trigger == RISING) || (state !=-1 && trigger == CHANGE))
        {
            ...
            #ifdef USE_SERIAL
            Serial.print("\nPCI");Serial.println(event);Serial.flush();
            #endif
            return true;
        }
    }
    ...
    while (1)
    {
        ...
        if (!event_triggered)
        {
            ...
            */
            ...
            {
                pir_is_off = false;
                //digitalWrite(node.PIRVccPin, HIGH); // not required. Panasonic PIR is always on.
                ...
                */
                ...
                Serial.print("pir_is_off: "); Serial.print(pir_is_off);
                Serial.print(", Flag_PIR_enable: "); Serial.print(node.Flag_PIR_enable);Serial.flush();
                Serial.print(", pir_inhibit_expired: "); Serial.print(pir_inhibit_expired);
                Serial.print(", status of PIRVccPin: "); Serial.print(digitalRead(node.PIRVccPin));Serial.flush();
                Serial.print(", interrupt enabled: "); Serial.print(is_pcint_enabled(node.PIRPin));
        ...
        if (watchdog_expired)
        {
            ...
            if(node.Flag_heartbeat_enable)
            {
                ...
                msg.brightness = brightness_with_LDR(node.LdrPin);
                #ifdef USE_HTU21D
                Measure_HTU21D(msg);
...
void onEvent (ev_t ev) {
    ...
    switch(ev) {
        ...
        case EV_TXCOMPLETE:
            ...
            Serial.print("LMIC.rxDelay: ");Serial.println(LMIC.rxDelay);
            #ifdef USE_SERIAL
            Serial.flush();
            #endif
            break;
...
{
    Serial.print("LMIC.seqnoUp = "); Serial.println(LMIC.seqnoUp);
    ...
    //Serial.print("LMIC.radio_txpow: "); Serial.println(LMIC.radio_txpow);
    Serial.flush();
}
...
void setup() {
    ...
    */
    /*
    Serial.print("LDR Pin: "); Serial.println(node.LdrPin);
...
/***  Initialize I2C and HTU21D  ***/
    ...
    LMIC.rxDelay = TTN_STACK_VERSION;
